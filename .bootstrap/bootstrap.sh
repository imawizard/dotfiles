...
go_clone_install() {
    ...
    local name=${pkg%%@*}
    #local mod=$(go list -m -json "$pkg")
    #local version=$(echo "$mod" | jq -r '.Version')
    ...
    git checkout "refs/tags/$version" 2>/dev/null
...
if [[ $(command -v flutter) ]]; then
    ...
    download=false
    docset=$(http --body --check-status https://master-api.flutter.dev/offline/flutter.xml 2>/dev/null)
    if [[ $? != 0 ]]; then
